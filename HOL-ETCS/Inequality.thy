...
proof -
  ...
  then have "\<exists> k. k \<in>\<^sub>c \<nat>\<^sub>c \<and> eq_pred \<nat>\<^sub>c \<circ>\<^sub>c \<langle>(add2  \<circ>\<^sub>c \<langle>k, m\<rangle>), (id \<nat>\<^sub>c \<circ>\<^sub>c n)\<rangle> = \<t>"
    by (typecheck_cfuncs, smt (verit, best) cfunc_cross_prod_comp_cfunc_prod cfunc_prod_type assms)
...
lemma fewer_is_less: 
  ...
  using assms unfolding leq_infix_def by (typecheck_cfuncs, metis add_commutes add_type exists_implies_leq_true leq_infix_def leq_transitivity)
...
\<rangle>)\<^sup>\<sharp> = (\<t> \<circ>\<^sub>c \<beta>\<^bsub>\<nat>\<^sub>c\<times>\<^sub>c\<nat>\<^sub>c\<^esub>)\<^sup>\<sharp>"
  proof(etcs_rule natural_number_object_func_unique[where X="\<Omega>\<^bsup>\<nat>\<^sub>c\<^esup>", where f = "id (\<Omega>\<^bsup>\<nat>\<^sub>c\<^esup>)"])
    ...
    proof(etcs_rule same_evals_equal[where Z = "\<one>", where X=\<Omega>, where A="\<nat>\<^sub>c"])
      ...
      proof(etcs_rule one_separator[where X="\<nat>\<^sub>c\<times>\<^sub>c \<one>", where Y = \<Omega>])
    ...
    id\<^sub>c (\<Omega>\<^bsup>\<nat>\<^sub>c\<^esup>) \<circ>\<^sub>c (\<t> \<circ>\<^sub>c \<beta>\<^bsub>\<nat>\<^sub>c \<times>\<^sub>c \<nat>\<^sub>c\<^esub>)\<^sup>\<sharp>"
      proof(etcs_rule same_evals_equal[where Z = "\<nat>\<^sub>c", where X = \<Omega>,where A = "\<nat>\<^sub>c"])
    ...
    eval_func \<Omega> \<nat>\<^sub>c \<circ>\<^sub>c id\<^sub>c \<nat>\<^sub>c \<times>\<^sub>f id\<^sub>c (\<Omega>\<^bsup>\<nat>\<^sub>c\<^esup>) \<circ>\<^sub>c (\<t> \<circ>\<^sub>c \<beta>\<^bsub>\<nat>\<^sub>c \<times>\<^sub>c \<nat>\<^sub>c\<^esub>)\<^sup>\<sharp>"
        proof(etcs_rule one_separator[where X="\<nat>\<^sub>c\<times>\<^sub>c\<nat>\<^sub>c", where Y = \<Omega>])
          ...
          proof -
            ...
            proof - 
              have calc1: "(eval_func \<Omega> \<nat>\<^sub>c \<circ>\<^sub>c id\<^sub>c \<nat>\<^sub>c \<times>\<^sub>f (\<t> \<circ>\<^sub>c \<beta>\<^bsub>\<nat>\<^sub>c \<times>\<^sub>c \<nat>\<^sub>c\<^esub>)\<^sup>\<sharp> \<circ>\<^sub>c successor) \<circ>\<^sub>c  x =
                ...
                by (typecheck_cfuncs, smt cfunc_type_def comp_associative inv_transpose_func_def3 inv_transpose_of_composition x_def)     
    ...
    (OR \<circ>\<^sub>c \<langle>leq \<circ>\<^sub>c \<langle>left_cart_proj \<nat>\<^sub>c \<nat>\<^sub>c,right_cart_proj \<nat>\<^sub>c \<nat>\<^sub>c\<rangle>,
         ...
         (eval_func \<Omega> \<nat>\<^sub>c \<circ>\<^sub>c id\<^sub>c \<nat>\<^sub>c \<times>\<^sub>f id\<^sub>c (\<Omega>\<^bsup>\<nat>\<^sub>c\<^esup>) \<circ>\<^sub>c
          ...
          successor) \<circ>\<^sub>c \<langle>n,m\<rangle>= 
                ...
                by (typecheck_cfuncs, smt cfunc_type_def comp_associative inv_transpose_func_def3 inv_transpose_of_composition m_def)
              also have "... = OR \<circ>\<^sub>c \<langle>leq \<circ>\<^sub>c \<langle>left_cart_proj \<nat>\<^sub>c \<nat>\<^sub>c,right_cart_proj \<nat>\<^sub>c \<nat>\<^sub>c\<rangle>,
                                    leq \<circ>\<^sub>c \<langle>right_cart_proj \<nat>\<^sub>c \<nat>\<^sub>c,left_cart_proj \<nat>\<^sub>c \<nat>\<^sub>c\<rangle>\<rangle>\<circ>\<^sub>c
                           (id\<^sub>c \<nat>\<^sub>c \<times>\<^sub>f successor) \<circ>\<^sub>c \<langle>n,m\<rangle>" 
...
proof(unfold leq_infix_def, rule ccontr)
  assume "k \<in>\<^sub>c \<nat>\<^sub>c \<and> leq \<circ>\<^sub>c \<langle>n,k\<rangle> = \<t> \<and> leq \<circ>\<^sub>c \<langle>k,successor \<circ>\<^sub>c n\<rangle> = \<t>"
  then have k_type[type_rule]: "k \<in>\<^sub>c \<nat>\<^sub>c" and  n_lt_k: "leq \<circ>\<^sub>c \<langle>n,k\<rangle> = \<t>" and k_lt_sn: "leq \<circ>\<^sub>c \<langle>k,successor \<circ>\<^sub>c n\<rangle> = \<t>"
    by (typecheck_cfuncs, linarith)
  assume "\<not> (k = n \<or> k = successor \<circ>\<^sub>c n)"
  then have k_not_n: "k \<noteq> n" and k_not_sn: "k \<noteq> successor \<circ>\<^sub>c n"
      by (typecheck_cfuncs, linarith)
...
proof - 
  ...
  next
    ...
    then show "Q \<circ>\<^sub>c successor \<circ>\<^sub>c n = \<t>"
       by (typecheck_cfuncs, metis Qn_true_eqs induction_case leq_infix_def lqe_antisymmetry nat_strict_total_order)
  qed
...
proof(rule ccontr) 
  ...
  qed
  have "\<not> nonempty S"
  proof(rule ccontr, clarify, unfold nonempty_def)
