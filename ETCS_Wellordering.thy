...
proof -
  ...
  obtain q where q_def: "q = (\<t> \<bowtie>\<^sub>f zero) \<circ>\<^sub>c case_bool \<circ>\<^sub>c \<chi>\<^sub>A \<circ>\<^sub>c zero"
    by simp
  ...
  obtain q' where q'_def: "q' = \<langle>\<chi>\<^sub>A \<circ>\<^sub>c zero, zero\<rangle>"
    by simp
